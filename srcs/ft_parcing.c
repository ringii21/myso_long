...
/*   Updated: 2022/05/25 14:44:05 by abonard          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (p > 1 || p == 0)
		return (ft_error("Only one player should be in this mofo map", game));
	if (c < 1)
		return (ft_error("MORE COINS, WE ARE GREEDY", game));
	game->num_c = c;
	if (e < 1)
		return (ft_error("Dont you wanna end the game?", game));
	return (0);
}
...
int	ft_parcing(t_map *game, char *file)
{
	...
	if (ft_check_ber(file) == 1)
		return (ft_error("this is not a ber file", game));
	fd = open(file, O_RDONLY);
	if (fd < 0)
		return (ft_error("Cannot open the file", game));
	lire = read(fd, buff, 5000);
	if (lire < 0 || lire > 5000)
		return (ft_error("Cannot read the file", game));
	game->map = ft_split(buff, '\n');
	if (ft_is_rectangle(game) == 1 || ft_surrounded_walls(game) == 1)
		return (ft_error("Map is not closed or rectangle", game));
