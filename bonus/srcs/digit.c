...
/*   Updated: 2022/05/25 15:24:10 by abonard          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_charge_number(t_map *game)
{
	...
	while (i < 10)
	{
		game->score[i] = mlx_xpm_file_to_image(game->mlix, path[i],
				&game->dx[i], &game->dy[i]);
		if (!game->score[i])
...
void	ft_put_digit(t_map *game)
{
	...
	if (dixaine == 1)
		mlx_put_image_to_window(game->mlix, game->window,
			game->score[game->num_comb], 0, 0);
	else if (dixaine == 2)
	{
		mlx_put_image_to_window(game->mlix, game->window,
			game->score[game->num_comb / 10], 0, 0);
		mlx_put_image_to_window(game->mlix, game->window,
			game->score[game->num_comb % 10], 1 * 32, 0);
	}
	else if (dixaine == 3)
	{
		mlx_put_image_to_window(game->mlix, game->window,
			game->score[game->num_comb / 100], 0, 0);
		dixaine = game->num_comb % 100;
		mlx_put_image_to_window(game->mlix, game->window,
			game->score[dixaine / 10], 1 * 32, 0);
		mlx_put_image_to_window(game->mlix, game->window,
			game->score[dixaine % 10], 2 * 32, 0);
	}
}
